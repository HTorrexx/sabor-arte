...
export function ProductsCard({ product, roles,section }) {
  ...
  const { token } = useAuth();
   const handleDelete = (id) => {
    toast((t) => (
      <div>
        <p className="text-white">Esta seguro de Eliminar? <strong>{id}</strong></p>
        <div>
          <button
  ...
  return (
    <div className={product.section === section ? "product-card" : "none"}>
      ...
      <button className={section === "nosotros" ? "none":"product-delete"}>Comprar</button>
      <button className={roles === "admin" ? "product-delete" : "none"} onClick={()=> navigate(`/products/${product._id}`)}>
        Actualizar
      </button>
    </div>
  );
